...
async function fetchData(username) {
  try {
    ...
    // Clear previous user info before inserting new
    const existingSection = mainContainer.querySelector(".about-user");
    if (existingSection) {
    ...
    const followButton = section.querySelector(".follow");
    followButton.addEventListener("click", () => {
      if (followButton.textContent === "Follow") {
        followButton.textContent = "Unfollow";
        followButton.style.backgroundColor = "#d9534f"; // red color for unfollow
...
async function fetchRepos(username) {
  try {
    ...
    // Clear previous repos before appending new
